...
char_to_childindex = {'A':0, 'C':1, 'G':2, 'T':3}
...
def subtree_values(root):
  if root.children != {}: # if root is not leaf
    result = []
    for char in root.children:
      result += subtree_values(root.children[char])
    return result
  else:
    return [root.value]

f = open('one_away_pairs.tsv', 'w')
g = open('coverages.tsv', 'w')

def insert(root, kmer, value):
  node = root
  index_last_char = None
  #k = len(kmer)
  possible_siblings = []
  for index_char, char in enumerate(kmer):
    #childindex = char_to_childindex[char]
#    if index_char >= k//2:
#      sibling_kmer_values = []
#      for siblingindex, sibling in node.children:
#        if siblingindex != childindex and sibling is not None:
#          sibling_kmer_values += subtree_values(sibling)
#      for sibling_kmer_value in sibling_kmer_values:
#        length_to_pairs[index_char].add((value,sibling_kmer_value))
    #possible_siblings = [possible_sibling.children[char] for possible_sibling in possible_siblings if char in possible_sibling.children] #follow possible siblings down one more character, and keep if match
    #possible_siblings += [node.children[sib_char] for sib_char in node.children if sib_char != char] #add possible siblings branching off from current character
  ...
  if index_last_char is not None:
    for char in kmer[index_last_char:]:
      #childindex = char_to_childindex[char]
      possible_siblings = [possible_sibling.children[char] for possible_sibling in possible_siblings if char in possible_sibling.children] #follow possible siblings down one more character, and keep if match
      possible_siblings += [node.children[sib_char] for sib_char in node.children if sib_char != char] #add possible siblings branching off from current character
  ...
  for possible_sibling in possible_siblings:
    f.write(str(possible_sibling.value) + '\t' + str(value) + '\n')
...
if __name__=='__main__':
  ...
  prefix_trie = Node()
  prefixlength_to_pairs = defaultdict(set)
  suffixlength_to_pairs = defaultdict(set)
  ...
  for i, line in enumerate(dumps_file):
    ...
    #insert(prefix_trie, kmer, i, prefixlength_to_pairs)
    #insert(suffix_trie, kmer[::-1], i, suffixlength_to_pairs)
    #insert(prefix_trie, kmer, i)
    #if i%1000==0:
    #  print(i)
    ...
    if new_kmer_L == current_kmer_L:
      #kmer_R = kmer[i_R_L+1:i_R_R+1] #changed... now kmer[i_R_L] is the middle nucleotide
      if kmer_R in kmer_R_to_index_family:
        for index,cov in kmer_R_to_index_family[kmer_R]:
          f.write(str(index) + '\t' + str(i) + '\n')
          g.write(str(cov) + '\t' + str(coverage) + '\n')
    else:
      current_kmer_L = new_kmer_L
      kmer_R_to_index_family = defaultdict(list)
    kmer_R_to_index_family[kmer_R].append((i,coverage))
  ...
  family_sizes = set()
  family_size_to_kmers = defaultdict(list)
  
  for kmer_L in kmer_L_to_index_family:
    family_size = len(kmer_L_to_index_family[kmer_L])
    family_sizes.add(family_size)
    family_size_to_kmers[family_size].append(kmer_L)

  #family_sizes.sort(reverse=True)
  #print(family_sizes[-5:])
  i = 0
  family_sizes = sorted(family_sizes, reverse=True)
  with open('family_sizes.txt','w') as f, open('common_kmers.txt','w') as g:
    for family_size in family_sizes:
      i+=1
      if i <= 50:
        g.write("%s\n" % family_size_to_kmers[family_size])
      f.write("%s\n" % family_size)
      #print(family_size_to_kmers[family_size])

  #max(family_sizes.items(), key=operator.itemgetter(1))[0]
  plt.hist(family_sizes)
  plt.savefig('sizes.png')
  plt.close()
  print(np.percentile(family_sizes, 85))
  sys.exit()
