...
filter_peaks <- function(.peak_points, .peak_sizes, threshold = 0.005){
    to_filter <- .peak_points$peak %in% which(.peak_sizes < threshold)
...
filter_peak_sizes <- function(.peak_sizes, threshold = 0.005){
    .peak_sizes[!peak_sizes < threshold]
}
...
# annotate peaks
annotate_peaks <- function(peak_points, total_pair_cov){
    min_kmerpair_cov <- min(total_pair_cov)
    max_cov_pair <- max(total_pair_cov)
    for (summit in which(peak_points$summit == T)){
        ...
        points(transform_x(to_plot$y), transform_y(to_plot$x, min_kmerpair_cov, max_cov_pair), pch = peak)
    }
}

# annotate summits
annotate_summits <- function(peak_points, peak_sizes, total_pair_cov, col = 'red'){
    min_kmerpair_cov <- min(total_pair_cov)
    max_cov_pair <- max(total_pair_cov)
    for (summit in which(peak_points$summit == T)){
         peak <- peak_points$peak[summit]
         ...
         text(transform_x(peak_points$y[summit]),
              transform_y(peak_points$x[summit], min_kmerpair_cov, max_cov_pair),
              round(peak_size, 3),
