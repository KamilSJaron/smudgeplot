...
class Smudges:
    ...
    def centrality_plot(self, output):
        ...
        fontsize=32
        plt.plot(self.centrality_df['coverage'],
...
class SmudgeplotData:
    ...
    def get_ax_lims(self, upper_ylim=None):
        if self.cov == np.percentile(a=self.cov_tab['total_pair_cov'], q=95, weights = self.cov_tab['freq'], method='inverted_cdf'):
            self.lims['ylim'] = [min(self.cov_tab['total_pair_cov']), max(self.cov_tab['total_pair_cov'])]
        else: 
            self.lims['ylim'] = [min(self.cov_tab['total_pair_cov']) - 1, # or 0?
...
def plot_smudges(cov_tab, colour_ramp, ax, lims, log=False, fontsize=14):
    ...
    cov_tab['col'] = [str(colour_ramp[int(i)]) for i in round((len(colour_ramp)-1) * cov_tab['freq'] / max(cov_tab['freq']))]
...
def plot_expected_haplotype_structure(smudge_tab, cov, ax, adjust=False, xmax=0.49):
    ...
    smudge_tab.loc[:,'corrected_minor_variant_cov'] = smudge_tab['structure'].str.count('B')/smudge_tab['ploidy']
...
def plot_smudge_sizes(smudge_tab, cov, error_string, ax):
    ax.plot()
    ax.set_title('')
    if cov > 0:
        ...
        ax.legend(loc='lower left', handles=[error_string],labels=[None], prop={'size': 18}, frameon=False)
...
def main():
    ...
    if _parser.task == "peak_aggregation":
        ...
        coverages.local_aggregation(distance = args.d, noise_filter = args.nf, mask_errors = False)
    ...
    if _parser.task == "all":
        ...
        smudges.final_smudges = smudges.get_smudge_container(
            cov,
            smudge_size_cutoff
        )
