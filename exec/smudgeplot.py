...
class Smudges:
    ...
    def get_centrality_df(self, min_c, max_c, smudge_size_cutoff=0.02):
        ...
        sys.stderr.write(
            f"Best coverage to precision of 1/{10**i} (just to be sure): {best_cov:.2f}\n"
        )
...
def get_centrality(smudge_container, cov, centre="mode", dist="theoretical_center"):
    ...
    for smudge, smudge_tab in smudge_container.items():
        ...
        if dist == "theoretical_edge":
            distA = min(
                abs(center["covA"] - (cov * (As - 0.5))), abs((cov * (As + 0.5)) - center["covA"])
            )
            distB = min(
                abs(center["covB"] - (cov * (Bs - 0.5))), abs((cov * (Bs + 0.5)) - center["covB"])
            )
...
def write_smudge_report(smudges, coverages, smudge_dict, cov, args, print_header):
    ...
    smudges.smudge_tab.loc[:, "label"] = reduce_structure_representation(
...
def main():
    ...
    if _parser.task == "peak_aggregation":
        ...
        coverages.local_aggregation(
            distance=args.d, noise_filter=args.nf, mask_errors=args.mask_errors
        )
